...
export function SearchResultsList({ results, loading, onClose, onResultClick }: SearchResultsListProps) {
  if (loading) {
    return (
      <div className="absolute top-20 left-4 right-4 z-[999] bg-[var(--bg-elevated)] rounded-2xl shadow-xl border border-[var(--border)] overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
          <h3 className="font-semibold text-sm text-[var(--fg)]">Searching...</h3>
          <button onClick={onClose} className="p-1.5 hover:bg-[var(--bg-hover)] rounded-full transition-colors">
  ...
  if (results.length === 0) {
    return (
      <div className="absolute top-20 left-4 right-4 z-[999] bg-[var(--bg-elevated)] rounded-2xl shadow-xl border border-[var(--border)] overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
          <h3 className="font-semibold text-sm text-[var(--fg)]">No shops found</h3>
          <button onClick={onClose} className="p-1.5 hover:bg-[var(--bg-hover)] rounded-full transition-colors">
            <X className="h-4 w-4 text-[var(--fg-muted)]" />
          </button>
        </div>
        <div className="p-6 text-center">
          <p className="text-sm text-[var(--fg-muted)]">Try a different search term or check your spelling.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="absolute top-20 left-4 right-4 z-[999] bg-[var(--bg-elevated)] rounded-2xl shadow-xl border border-[var(--border)] max-h-[60vh] overflow-hidden flex flex-col">
      <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
        <h3 className="font-semibold text-sm text-[var(--fg)]">{results.length} shops found</h3>
        <button onClick={onClose} className="p-1.5 hover:bg-[var(--bg-hover)] rounded-full transition-colors">
          <X className="h-4 w-4 text-[var(--fg-muted)]" />
        </button>
      </div>
      <div className="overflow-auto flex-1">
        {results.map((shop) => (
          <div key={shop.shop_id} onClick={() => onResultClick?.(shop)}
            className="flex items-center gap-3 p-4 border-b border-[var(--border)] hover:bg-[var(--bg-hover)] cursor-pointer transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-sm text-[var(--fg)] truncate">{shop.name}</h4>
